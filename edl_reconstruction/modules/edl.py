...
class NormalInvGammaConv2d(nn.Module):
    """
    Convolutional version of NormalInvGamma
    """
    def __init__(self, in_channels, out_channels, kernel_size=1, padding='same'):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels, 
            out_channels * 4, # because we need to output 4 parameters per channel
    ...
    def forward(self, x):
        out = self.conv(x)
        mu, logv, logalpha, logbeta = torch.split(out, self.out_channels, dim=1)
        v = self.evidence(logv)
        alpha = self.evidence(logalpha) + 1
        beta = self.evidence(logbeta)
        return mu, v, alpha, beta
